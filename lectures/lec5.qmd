...
__Theorem (Schmugden'1991)__  
  ...
  - If $p = \sum_{q \in Q(S)} \sigma_q q$ for SOS polynomials $\sigma_q$ then $p_1(\vec{x}) \geq 0\ \land\ \cdots\ \land\ p_m(\vec{x}) \geq 0 \models p \geq 0$.
  - Conversely, if $p_1(\vec{x}) \geq 0\ \land\ \cdots\ \land\ p_m(\vec{x}) \geq 0 \models p > 0$ then $p$ can be written as a SOS-combination of polynomials in $Q(S)$.
